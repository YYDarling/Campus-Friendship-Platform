...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="公告"><template #default="scope"><span v-if="scope.row.noticeId">{{ state.noticeOptions.find(v => v.id === scope.row.noticeId) ? state.noticeOptions.find(v => v.id === scope.row.noticeId).name : '' }}</span></template></el-table-column>
      <el-table-column prop="hideRadio" label="是否隐藏"></el-table-column>
      <el-table-column prop="date" label="日期"></el-table-column>
      <el-table-column prop="time" label="时间"></el-table-column>
      <el-table-column label="文件"><template #default="scope"> <a :href="scope.row.file" target="_blank" style="text-decoration: none; color: dodgerblue">点击下载</a></template></el-table-column>
    ...
    />
        ...
        <el-form-item prop="noticeId" label="公告">
          <el-select clearable v-model="state.form.noticeId" placeholder="请选择"  style="width: 100%">
            <el-option v-for="item in state.noticeOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="hideRadio" label="是否隐藏">
          <el-radio-group v-model="state.form.hideRadio">
            <el-radio label="是">是</el-radio>
            <el-radio label="否">否</el-radio>
          </el-radio-group>        </el-form-item>
        <el-form-item prop="date" label="日期">
          <el-date-picker style="width: 100%" v-model="state.form.date" type="date" value-format="YYYY-MM-DD" placeholder="选择日期"></el-date-picker>
        </el-form-item>
        ...
        <el-form-item prop="file" label="文件">
          <el-upload :show-file-list="false" :action="`http://${config.serverUrl}/file/upload`" ref="file" :headers="{ Authorization: token}" :on-success="handleFileUploadSuccess">
            <el-button size="small" type="primary">点击上传</el-button>
          </el-upload>
        </el-form-item>
